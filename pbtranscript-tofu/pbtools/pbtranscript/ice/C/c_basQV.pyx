...
cpdef precache_helper(char* bas_file, list seqids, list QV_names, dict qv_dict):
    ...
    #print >> sys.stderr, "worker precaching {0} ids from {1}".format(len(seqids), bas_file)
    for seqid in seqids: # must be in the same bas file
        seqid = seqid.split()[0]  # in case there is extra information like fiveseen=1;threeseen=0;
