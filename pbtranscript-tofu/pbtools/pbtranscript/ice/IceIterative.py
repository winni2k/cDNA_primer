...
from pbtools.pbtranscript.Utils import mknewdir
...
#from pbtools.pbtranscript.ice.ProbModel import ProbFromQV
...
class IceIterative(IceFiles):
    ...
    def __init__(self, fasta_filename,
        ...
        ice_opts, sge_opts,
        uc=None, probQV=None,
        refs=None, d=None, is_FL=True, qv_prob_threshold=.1):
        #ece_penalty=1, ece_min_len=20, maxScore=-1000,
        #job_random_offset=random.randint(1, 9999)
        #, cpus=24
        """
        ...
        self.gcon_nproc = sge_opts.gcon_nproc
        self.job_random_offset = sge_opts.unique_id
    ...
    def tmpConsensusFa(self):
        ...
        """
        return op.join(self.out_dir, "tmp.consensus.fa")
    ...
    def refConsensusFa(self):
        ...
        return op.join(self.root_dir, "ref_consensus.fa")

    @property
    def tmpOrphanFa(self):
        """Return $root_dir/tmp.orphan.fa, a Fasta file of orphan sequences."""
        return op.join(self.root_dir, "tmp.orphan.fa")
    ...
    def uptoConsensusFa(self, fa):
        """Given a Fasta file: path_to_fa/*.fa, return
           $self.out_dir/upto_*.fa.consensus.fa
        """
        fn = fa.split('/')[-1]
        return op.join(self.out_dir, fn + ".consensus.fa")
    ...
    def write_final_consensus(self):
        ...
        """
        msg = "Writing final consensus isoforms to {f}".format(
            f=self.final_consensus_fa)
        self.add_log(msg)
        self.write_consensus(self.final_consensus_fa)
        ...
        with open(fasta_filename, 'w') as f:
            for cid, filename in self.refs.iteritems():
                ...
                newid = "c{cid}/{ab}/{le}".format(cid=cid,
                    ab=len(self.uc[cid]), le=len(r.sequence))
                f.write(">{0}\n{1}\n".format(newid, r.sequence))
    ...
    def write_pickle(self, pickle_filename):
        ...
        with open(pickle_filename, 'w') as f:
            d = {'uc':self.uc,
                 ...
                 'all_fasta_fiilename':self.all_fasta_filename,
...
#        _failedTimes = dict((cid, 0) for cid in self.unrun_cids)
...
#            for _cid in _cids:
#                if _failedTimes[_cid] >= 10:
#                    errMsg = "gcon for cluster {0} failed 10 times, abort!".\
#                        format(_cid)
#                    self.add_log(errMsg, level=logging.ERROR)
#                    raise RuntimeError(errMsg)
#                _failedTimes[_cid] += 1
    ...
    def run_gcon_parallel_helper(self, cids):
        ...
        for cid in cids:
            ...
            #op.join("./tmp/", str(cid/10000), 'c'+str(cid))
            ...
            if len(self.uc[cid]) <= 2: # don't even bother running gcon
                ...
                argstr = " {infa}".format(infa=in_fa_filename) + \
                         " {cdir}/g_consensus".format(cdir=self.cluster_dir(cid)) + \
                         " c{i}".format(i=cid) + \
                         " --nproc {nproc}".format(nproc=self.gcon_nproc) + \
        ...
        if effective_i > 0:
            ...
            if self.use_sge is True:
                ...
                for job_i, f in job_sh_dict.iteritems():
                    ...
                    jid = "j_{it}_{j}_{o}".format(it=self.iterNum,
                    j=job_i, o=self.job_random_offset)
                    elog = self.elogFN(self.iterNum, f.name)
                    olog = self.ologFN(self.iterNum, f.name)

                    cmd = "qsub -pe smp {nproc} -S /bin/bash -V -cwd ".format(nproc=self.gcon_nproc) + \
                          "-e {elog} ".format(elog=elog) + \
                          "-o {olog} ".format(olog=olog) + \
                          "-N {jid} ".format(jid=jid) + \
                          "{fn}".format(fn=f.name)
                          #"-e /dev/null -o /dev/null " + \
                    #subprocess.check_call("qsub -pe smp 1 -cwd -S /bin/bash " +
                    #"-e /dev/null -o /dev/null  -N j{0} {1}".format(
                    #job_i+self.job_random_offset,f.name), shell=True)
                    _out, _code, _msg = backticks(cmd)
                    self.add_log("Submitting CMD: {cmd}.\n".format(cmd=cmd))
                    job_list += jid + ','

                with open(self.gconDoneJobFN(self.iterNum), 'w') as f:
                    f.write("touch {alohaFN}\n".format(
                        alohaFN=self.alohaFN(self.iterNum)))
                ...
                cmd = "qsub -sync y -pe smp 1 -S /bin/bash -V -cwd " + \
                      ...
                      "-N qcon_done_{r} ".format(r=self.job_random_offset) + \
                      "{donesh}".format(donesh=self.gconDoneJobFN(self.iterNum))
                      # "-cwd " + \
                      #"-e /dev/null -o /dev/null " + \
                _out, _code, _msg = backticks(cmd)
                #subprocess.check_call("qsub -sync y -pe smp 1 -cwd "
                # "-S /bin/bash -e /dev/null -o /dev/null -hold_jid "
                # "{0} ./scripts/gcon_done.sh".
                # format(job_list[:-1]), shell=True)
            else:
                for job_i, f in job_sh_dict.iteritems():
                    f.close()

                for job in jobs:
                    msg = "Adding job to POOL: " + self.gcon_py + job
                    self.add_log(msg)

                if len(jobs) > 0:
                    pool = Pool(processes=self.gcon_nproc)
                    rets = pool.map(runConsensus, jobs)
                    pool.close()
                    pool.join()
                    # Check whether all ice_gcontools threads finished successfully
                    for i, job in enumerate(jobs):
                        if rets[i] != 0:
                            errMsg = "Job {j} failed.".format(j=job)
                            self.add_log(errMsg, level=logging.ERROR)
                            raise RuntimeError (errMsg)
            #else:  # Run jobs sequentially
            #    for job_i, f in job_sh_dict.iteritems():
            #        f.close()
            #        self.add_log("running script: " + f.name)
            #        _out, _code, _msg = backticks("bash " + f.name)
        ...
        #if effective_i > 0 and self.use_sge:
        #    os.remove(self.gconDoneJobFN(self.iterNum))
        #    e.g., "./scripts/0/gcon_done.sh")
        #    os.remove(self.alohaFN(self.iterNum))
        #    e.g., "./scripts/0/aloha")
    ...
    def choose_ref_file(self, cid):
        ...
        if op.exists(cons) and os.stat(cons).st_size > 0:
            ...
            with open(rname, 'w') as h:
                h.write(">c{0}\n{1}\n".format(cid,
                    self.seq_dict[self.uc[cid][0]].sequence))
            return rname
    ...
    def calc_cluster_prob(self, force_calc=False):
        ...
        saFN = f.name + ".sa"
        cmd = "sawriter {sa} {fa} -blt 8 -welter".format(sa=saFN, fa=f.name)
        self.add_log("Creating suffix array for ref_consensus.fa.")
        self.add_log("CMD: {cmd}".format(cmd=cmd))
        #subprocess.check_call(cmd, shell=True)
        _out, _code, _msg = backticks(cmd)
        if _code != 0:
            errMsg = "Failed to create suffix array: {cmd}".format(cmd)
            self.add_log(errMsg, level=logging.ERROR)
            raise RuntimeError(errMsg)
        ...
        cmd = "blasr {qfa} ".format(qfa=self.fasta_filename) + \
              "{tfa} ".format(tfa=f.name) + \
              "-m 5 -bestn 10 -nCandidates 10 -maxLCPLength 15 " + \
              "-nproc {n} ".format(n=self.blasr_nproc) + \
              "-maxScore {s} ".format(s=self.maxScore) + \
              "-sa {sa} -out {o}".format(sa=saFN, o=blasrFN)
        ...
        _out, _code, _msg = backticks(cmd)
        if _code != 0:
            errMsg = "Failed to run blasr: {cmd}".format(cmd)
            self.add_log(errMsg, level=logging.ERROR)
            raise RuntimeError(errMsg)
        ...
        for hit in blasr_against_ref(
            ...
            if hit.fakecigar is not None:
                self.d[hit.qID][hit.cID] = self.probQV.calc_prob_from_aln(
                    hit.qID, hit.qStart, hit.qEnd, list(hit.fakecigar))
        ...
        while no_change_count < 10 and iter_count <= max_iter:
            ...
            for cid in self.changes:
                if cid in self.refs:
                    ...
                    #        SeqIO.read(h, 'fasta').seq.tostring()
                    current_gcon_seq_in_changes[cid] = \
    ...
    def add_new_batch(self, batch_filename):
        ...
        # sanity check:
        # the file should exist & it should not already exist
        ...
        #for r in SeqIO.parse(open(batch_filename), 'fasta'):
        ...
        cmd = "cp {bat} {cur}".format(bat=batch_filename,
                                      cur=self.fasta_filename)
        _out, _code, _msg = backticks(cmd)
        if _code != 0:
            errMsg = "Error copying files: " + cmd
            self.add_log(errMsg, level=logging.ERROR)
        ...
        with FastaReader(self.fasta_filename) as reader:
            for r in reader:
                ...
                try:
                    ...
                    #has_err = False
                    self.probQV.add_ids_from_fasta(newids=[rid], _smooth=True)
    ...
    def find_mergeable_consensus(self, fasta_filename):
        ...
        if op.exists(out): # clean out the blasr file from the last run
            os.remove(out)
        cmd = "blasr {i} {i} ".format(i=fasta_filename) + \
              "-bestn 10 -nCandidates 10 -minPctIdentity 99 -m 5 " + \
              "-maxScore {s} ".format(s=self.maxScore) + \
              "-nproc {cpu} -out {o}".format(cpu=self.blasr_nproc, o=out)
        self.add_log("Calling blasr to self align " + fasta_filename)
        self.add_log("CMD: " + cmd)
        _out, _code, _msg = backticks(cmd)
        if _code != 0:
            errMsg = "Failed to run blasr: " + cmd
            self.add_log(errMsg, level=logging.ERROR)
            raise RuntimeError(errMsg)
    ...
    def write_report(self, uc, report_fn):
        """
    ...
    def run(self):
        ...
        """
        msg = "IceIterative starts to run."
        self.add_log(msg)
       ...
       # Write final pickle
        ...
        self.write_report(uc=self.uc, report_fn=self.report_fn)
