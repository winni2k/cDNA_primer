...
import BioReaders
import c_branch
from modified_bx_intervals.intersection_unique import IntervalTreeUnique
...
class BranchSimple:
    ...
    """
    def __init__(self, transfrag_filename, cov_threshold=2, min_aln_coverage=.99, min_aln_identity=.85):
    ...
    def iter_gmap_sam(self, gmap_sam_filename, ignored_fout):
        ...
        for r in iter:
            ...
            if r.qCoverage < self.min_aln_coverage:
                ...
                ignored_fout.write("{0}\tIdentity {1:.3f} too low.\n".format(r.qID, r.identity))
            elif r.sID != records[0].sID or r.sStart > records[-1].sEnd:
